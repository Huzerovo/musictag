...
void extract_image(TagLib::FileRef fr, const char *extract_to) {
  ...
  if (!output.is_open()) {
    std::cerr << "extract_image: Can not open file " << extract_to << std::endl;
    return;
  }
  output << data;
  output.close();
...
void import_image(TagLib::FileRef *fr, const char *import_from) {
  ...
  if (!img.is_open()) {
    std::cerr << "import_image: Can not open file " << import_from << std::endl;
    return;
  }
...
int main(int argc, char **argv) {
  ...
  while (1) {
    int option_idx = 0;
    c = getopt_long(argc, argv, "i:a:t:c:E:Lhv", options_map, &option_idx);
    if (c == -1)
      break;
    switch (c) {
    case 0:
  ...
  if (flag_list) {
    std::cout << "Title: " << fr.tag()->title().toCString(true) << std::endl;
    std::cout << "Artist: " << fr.tag()->artist().toCString(true) << std::endl;
